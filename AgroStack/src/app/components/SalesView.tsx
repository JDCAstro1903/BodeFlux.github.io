...
export function SalesView() {
  ...
  const addToSupplierRequest = (productId: string) => {
    setSupplierRequests(prev => ({
      ...prev,
      [productId]: (prev[productId] || 0) + 10, // Default 10 units
    }));
  };
  ...
  const removeFromSupplierRequest = (productId: string) => {
    setSupplierRequests(prev => {
      ...
      if (newRequests[productId] > 10) {
        newRequests[productId] -= 10;
      } else {
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {supplierRequestItems > 0 && (
              <button
                ...
                className="px-4 py-2 rounded-[12px] bg-gradient-to-br from-[#1B4332] to-[#2D6A4F] text-white hover:shadow-lg transition-all flex items-center gap-2"
                style={{ fontSize: '13px', fontWeight: '600' }}
              >
                <Send size={16} />
                Enviar Solicitud
            ...
            <div className="space-y-2">
              {Object.entries(supplierRequests).map(([productId, quantity]) => {
                ...
                return (
                  <div
                    key={productId}
                    className="rounded-[16px] bg-white/60 p-4 flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '12px', color: '#6B7280' }}>
                          Solicitar: {quantity} {product.unit}
                        </div>
                      </div>
                    </div>
                    <button
                      onClick={() => removeFromSupplierRequest(productId)}
                      className="px-3 py-1 rounded-full bg-[#EF4444]/10 text-[#EF4444] hover:bg-[#EF4444]/20 transition-all"
