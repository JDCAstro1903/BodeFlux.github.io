import { Building2, Phone, Mail, MapPin, Plus, Edit, Trash2, Star, Package, X } from 'lucide-react';
import { useState, useEffect } from 'react';
import { providerApi, productApi, type ProviderAPI, type ProductAPI } from '../services/api';
...
export function ProviderManagement() {
  ...
  const handleAssignProduct = async (provider: Provider) => {
    if (!selectedProductId) return;
    try {
      const updated = await productApi.update(selectedProductId as number, {
        provider_id: provider.numericId,
  ...
  const getProviderProducts = (providerId: number) =>
    products.filter((p) => p.provider_id === providerId);

  const unassignedProducts = products.filter((p) => !p.provider_id);
      ...
      <div className="space-y-2 md:space-y-3">
        ...
        {providers.map((provider) => (
          ...
          >
            ...
            {/* Products assigned to this provider */}
            <div className="mt-3 pt-3 border-t border-gray-200/50 dark:border-gray-600/50">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  <Package size={12} className="text-[#6B7280] dark:text-[#CBD5E1]" />
                  <span className="text-xs font-semibold text-[#6B7280] dark:text-[#CBD5E1] uppercase tracking-wide">
                    Productos ({getProviderProducts(provider.numericId).length})
                  </span>
                </div>
                <button
                  onClick={() => {
                    setAssigningProviderId(assigningProviderId === provider.numericId ? null : provider.numericId);
                    setSelectedProductId('');
                  }}
                  className="text-xs font-semibold text-[#0071E3] dark:text-[#60A5FA] hover:underline flex items-center gap-1"
                >
                  <Plus size={11} />
                  Asignar
                </button>
              ...
              {/* Assign product row */}
              {assigningProviderId === provider.numericId && (
                <div className="flex gap-2 mb-2">
                  <select
                    value={selectedProductId}
                    onChange={(e) => setSelectedProductId(e.target.value === '' ? '' : Number(e.target.value))}
                    className="flex-1 px-2 py-1.5 rounded-[10px] border border-gray-200 dark:border-gray-600 bg-white dark:bg-[#1E293B] text-[#1B4332] dark:text-[#F1F5F9] focus:outline-none text-xs"
                  >
                  ...
                  <button
                    onClick={() => handleAssignProduct(provider)}
                    disabled={!selectedProductId}
                    className="px-3 py-1.5 rounded-[10px] bg-[#0071E3] dark:bg-[#3B82F6] text-white text-xs font-semibold disabled:opacity-40"
                  >
                    OK
                  </button>
                </div>
              ...
              {getProviderProducts(provider.numericId).length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                  {getProviderProducts(provider.numericId).map((p) => (
                    <span
                      key={p.id}
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-[#1B4332]/10 dark:bg-[#34D399]/20 text-xs font-medium text-[#1B4332] dark:text-[#34D399]"
                    >
                ...
                <p className="text-xs text-[#6B7280] dark:text-[#CBD5E1] italic">
